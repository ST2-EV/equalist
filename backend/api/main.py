# from create_playlist import make
# from db import checkUserExists, getFromRooms, getFromRoomsByJoinCode, getFromUsers, roomsDb, updateRoom, updateRoomId
from typing import Optional
# from helpers import jsonEncode, newRoom, newUser, newUserAlt, randomString, fetchAndModify
# from spotify import AUTH_URL, CLIENT_ID, REDIRECT_URI, get_access_and_refresh_token
from fastapi import FastAPI
from fastapi.middleware.cors import CORSMiddleware
from fastapi.responses import RedirectResponse, HTMLResponse, JSONResponse
# from urllib.parse import urlencode
# from models import Confirm, Connect, Join, Login, Auth, Room
...
#     except Exception as e:
#         print(e)
#         return jsonEncode({"message": "Oops! couldn't get the room's information! (It's our fault, come back later)"}, 500)
...
#     except Exception as e:
#         print(e)
#         return jsonEncode({"message": "Oops! couldn't create the playlist (It's our fault, come back later)"}, 500)
...
#     '''
#     try:
#         query = next(roomsDb.fetch({"join_code": item.join_code}))
#         room = getFromRooms(query[0]['key'])
#         if room['active']:
#             robj = {
#                 "name": room['name'],
#                 "room_id": room['key'],
...
#         room = list(roomsDb.fetch({"join_code": join_code}))[0][0]
#         current_room = getFromRooms(room["key"])

#         if not user["key"] in current_room["people"]:
#             updateRoom(user["key"], room["key"])
#             return jsonEncode({"message": "joined room", "user_id": user, "room_id": room["key"]}, 200)
#         else:
#             return jsonEncode({"message": "joined room", "user_id": user, "room_id": room["key"]}, 200)
#     except Exception as e:
